...
// TODO: Make polish_stack_t.stack a dynamic array.
typedef struct polish_stack {
		int stack_pointer;
		int stack[MAXSIZE];
	} polish_stack_t;

// TODO: Make stack_push check if polish-stack_t.stack has available space. If not realloc.
...
int main(void) {
	polish_stack_t stack = {0};
	stack_push(&stack, 10,2);
	stack_print(&stack);
	stack_div(&stack);
	stack_print(&stack);
	return 0;
